new Vue({
  ...
  methods: {
      random_code: function  () {
          ...
          var random= Math.random().toString(36).substr(10);
          var res= random.concat(end);

          return res
      },
      validate: function () {
        if (this.repassword != this.password || this.name == '' || this.email == '' || this.wpno == '' || this.pno == '' || this.password == '' && this.college == '') {
            if (this.name == '') {
                alert('Name is empty');
            }
            if (this.username == '') {
        ...
        firebase.firestore().collection('campus_ambassadors').where('username', '==', this.username)
        .get()
      ...
      register () {
        ...
        if (!result) {
          ...
          firebase.firestore().collection('campus_ambassadors').where('referralcode', '==', referralcode)
          .get()
          .then(function (querySnapshot) {
          ...
          .then(function (user) {
            ...
            firebase.firestore().collection('campus_ambassadors').doc(user.user.uid).set(data)
            .then(function () {
